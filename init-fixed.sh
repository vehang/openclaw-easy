...
echo "=== OpenClaw 初始化脚本 (Easy Docker 修复版) ==="
...
GATEWAY_BIND="${OPENCLAW_GATEWAY_BIND:-$(jq -r '.gateway.bind // "lan"' ~/.openclaw/openclaw.json 2>/dev/null || echo 'lan')}"
GATEWAY_PORT="${OPENCLAW_GATEWAY_PORT:-$(jq -r '.gateway.port // 18789' ~/.openclaw/openclaw.json 2>/dev/null || echo '18789')}"
GATEWAY_TOKEN="${OPENCLAW_GATEWAY_TOKEN:-$(jq -r '.gateway.auth.token // ""' ~/.openclaw/openclaw.json 2>/dev/null)}"

# 如果 token 为空，生成一个随机 token
if [ -z "$GATEWAY_TOKEN" ]; then
    GATEWAY_TOKEN=$(cat /proc/sys/kernel/random/uuid 2>/dev/null || echo "default-token-$(date +%s)")
    echo "⚠️ Gateway Token 未配置，已自动生成随机 token"
fi

echo "Gateway 实际配置: bind=$GATEWAY_BIND, port=$GATEWAY_PORT, token=${GATEWAY_TOKEN:0:8}..."
