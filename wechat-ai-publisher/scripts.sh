...
create_cover_file() {
    ...
    local prompt=$1
    local article_id=$2
    local tmp_file="$HOME/.openclaw/workspace/screenshots/wechat_cover_${article_id}.png"
...
publish_article() {
    ...
    # 4. 创建草稿
    echo "4. 创建草稿..."
    local draft_json="$HOME/.openclaw/workspace/screenshots/draft_${article_id}.json"
    jq -n \
        --arg title "$title" \
        --arg author "$author" \
        --arg digest "$digest" \
