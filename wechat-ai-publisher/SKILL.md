...
- 风格：github
- 配图：封面 + 3 张
- 作者：田威 AI（环境变量 `WECHAT_AUTHOR` 默认值）
...
1. 如果用户/定时任务消息里出现 `主题=...` / `风格=...` / `配图=...` / `作者=...`，则覆盖默认值。
2. 其余自然语言描述（例如“写得更科普一些”“多强调边界和风险”“更偏护理视角”）视为写作约束，AI 可以自由调整结构与措辞以提升可读性。
3. 冲突时以用户指令为准；不确定就保守处理（不编造数据、不夸大结论）。
...
python3 ~/.codex/skills/article-illustrator/scripts/generate_image.py \
  --prompt "封面图描述，中文标注" \
  --output "$HOME/.openclaw/workspace/screenshots/wechat_cover.png" \
  --aspect-ratio "16:9"

# 生成配图（4:3，适合文章内嵌）
python3 ~/.codex/skills/article-illustrator/scripts/generate_image.py \
  --prompt "配图描述，中文标注" \
  --output "$HOME/.openclaw/workspace/screenshots/wechat_img1.png" \
  --aspect-ratio "4:3"
```
...
with open(os.path.expanduser("~/.openclaw/workspace/screenshots/wechat_cover.png"), "rb") as f:
...
#    返回的 URL 可以直接嵌入 HTML
with open(os.path.expanduser("~/.openclaw/workspace/screenshots/wechat_img1.png"), "rb") as f:
