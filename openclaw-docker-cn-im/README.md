...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `WORKSPACE` | 工作空间目录 | `/root/.openclaw/workspace` |
...
2. 挂载该目录到容器：`-v ~/.openclaw:/root/.openclaw`
...
容器首次启动时，如果 `/root/.openclaw/openclaw.json` 不存在，初始化脚本会根据环境变量自动生成配置文件，包括：
