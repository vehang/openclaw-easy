...
remove_nim_from_config() {
    ...
    python3 -c "
import json, os
cfg = os.environ.get('_CONFIG_FILE')
try:
    ...
    print(f'  remove_nim_from_config error: {e}')
"
}
...
try:
    ...
    print(f'  ensure_nim_in_config error: {e}')
"
}

# ========== 检查 nim 插件是否已安装（直接检查目录，不依赖 openclaw 命令） ==========
is_nim_installed() {
...
# 先确保配置中没有 nim 引用，避免 openclaw 命令报错
remove_nim_from_config
...
if is_nim_installed; then
    ...
    if openclaw plugins install "openclaw-nim-yx-auth@${NIM_PLUGIN_VERSION}" 2>&1; then
        ...
        # fallback: 直接 npm 安装到 extensions 目录
...
# 现在同步配置（此时 nim 插件已安装，配置写入 nim 引用不会报错）
sync_config_with_env
