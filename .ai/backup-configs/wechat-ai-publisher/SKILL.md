...
**配置文件**：`/root/.openclaw/skills/wechat-ai-publisher/accounts.json`
...
**使用方式**：
- 默认账号：`发布AI热点` → 使用 default 指定的账号
- 指定账号：`用 tech 账号发布AI热点`

### 图片生成 API（可选）

- **模型**: `google/nano-banana-pro`（通过 Replicate）
- **配置**: 在 `.env` 文件中设置 `REPLICATE_API_KEY`

> 如无图片生成 API，可跳过配图步骤，文章仍可发布
