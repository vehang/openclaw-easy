...
ensure_config_persistence() {
    ...
    # 4. 权限修复
    if is_root; then
        chown -R root:root "$persistent_config_dir" || true
        chown -h root:root "$container_config_dir" || true
    fi
}
...
fix_permissions_if_needed() {
    ...
    echo "当前所有者(UID:GID): $current_owner"
        ...
        chown -R root:root "$OPENCLAW_HOME" || true
    fi
...
CHANNEL_INSTALLS = {
    'feishu': {'source': 'npm', 'spec': '@openclaw/feishu', 'installPath': '/root/.openclaw/extensions/feishu'},
    'dingtalk': {'source': 'npm', 'spec': 'https://github.com/soimy/clawdbot-channel-dingtalk.git', 'installPath': '/root/.openclaw/extensions/dingtalk'},
    'openclaw-qqbot': {'source': 'path', 'sourcePath': '/root/.openclaw/openclaw-qqbot', 'installPath': '/root/.openclaw/extensions/openclaw-qqbot'},
    'napcat': {'source': 'path', 'sourcePath': '/root/.openclaw/extensions/napcat', 'installPath': '/root/.openclaw/extensions/napcat'},
    'wecom': {'source': 'npm', 'spec': '@sunnoy/wecom', 'installPath': '/root/.openclaw/extensions/wecom'},
}
...
def sync_models(ctx):
    ...
    workspace_root = (ctx.env.get('OPENCLAW_WORKSPACE_ROOT') or '/root/.openclaw').rstrip('/') or '/'
...
def migrate_qqbot_plugin_entry(ctx):
    ...
    if isinstance(legacy_install, dict):
        if not isinstance(official_install, dict):
            ...
            migrated_install['sourcePath'] = '/root/.openclaw/openclaw-qqbot'
            migrated_install['installPath'] = '/root/.openclaw/extensions/openclaw-qqbot'
