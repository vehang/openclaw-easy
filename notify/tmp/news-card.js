...
const CONFIG = {
  ...
  rssFeeds: [
    ...
    { name: '36氪', url: 'https://36kr.com/feed', fallbacks: ['https://hub.slarker.me/36kr/newsflashes'] }
  ],
  keywords: ['AI', '英伟达', '芯片', '半导体', '大模型', 'DeepSeek', 'OpenAI', '特斯拉', '苹果', '微软', '谷歌', 'Anthropic', '月之暗面', 'MiniMax', '存储', '新能源'],
  // 去重配置
  sentNewsFile: '/home/node/.openclaw/workspace/scripts/sent-news.json',
...
// 获取 RSS 内容（支持重定向和 User-Agent，带超时）
function fetchRSSOnce(url, timeoutMs = 20000) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(url, {
      ...
      if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
        ...
        return fetchRSSOnce(next, timeoutMs).then(resolve, reject);
      }
...
async function fetchRSS(url, fallbacks = []) {
  ...
  for (const candidate of candidates) {
    try {
      const xml = await fetchRSSOnce(candidate);
      if (xml && xml.includes('<item')) return xml;
      throw new Error('内容不含 RSS item');
    } catch (e) {
      lastErr = e;
    }
  }
  ...
  try {
    ...
    for (const feed of CONFIG.rssFeeds) {
      try {
        ...
        const xml = await fetchRSS(feed.url, feed.fallbacks || []);
